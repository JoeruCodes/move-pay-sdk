...
export function createQRCode(url: string, size = 512, background = 'black', color = 'yellow'): QRCodeStyling{
    return new QRCodeStyling(createQROptions(url, size, background, color));
}
function createQROptions(url: string | URL, size = 512, background = 'black', color = 'yellow'): Options {
    return {
        type: 'svg',
        width: size,
        height: size,
