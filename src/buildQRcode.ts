...
export function createQRCode(url: string, size = 512, background = 'white', color = 'black'): QRCodeStyling{
    return new QRCodeStyling(createQROptions(url, size, background, color));
}

function createQROptions(url: string | URL, size = 512, background = 'white', color = 'black'): Options {
    return {
        type: 'svg',
        width: size,
        height: size,
