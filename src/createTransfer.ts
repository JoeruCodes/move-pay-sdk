import {AccountAddress, AccountAddressInput, Aptos, AptosConfig, HexInput, InputGenerateTransactionOptions, InputGenerateTransactionPayloadData, MoveUint64Type, MoveUint8Type, MoveValue, Network } from "@aptos-labs/ts-sdk";
import { Amount, Recipient, Token } from "./types";
import BigNumber from "bignumber.js";
import { assert } from "console";
import { fixMetadata, isAccountPrimaryStoreFrozen, primaryStoreExists, resolveTokenAddress } from "./utils";
...
export async function createTransfer(
    ...
    const tokenMetadata: MoveValue[] = await resolveTokenAddress(token, client, tokenSymbol);
    const payload: RawJsonPayload = {
        data:{
            function: "0x1::primary_fungible_store::transfer",
            typeArguments: ["0x1::fungible_asset::Metadata"],
            functionArguments: [fixMetadata(tokenMetadata), recipent, amount.toNumber()],
        },
        withFeePayer: true,
        message
    };
...
export async function createSystemInstruction(client: Aptos, recipent: AccountAddressInput, amount: BigNumber, message?:string): Promise<RawJsonPayload> {
    const recipientInfo = await client.getAccountInfo({accountAddress: recipent});
    if (!recipientInfo) throw new CreateTransferError('recipient not found');
...
export async function createCustomTokenInstruction(recipent: AccountAddressInput, amount: BigNumber, customToken: AccountAddressInput, client: Aptos, symbol?: string, message?: string): Promise<RawJsonPayload>{
    const recipientInfo = await client.getAccountInfo({accountAddress: recipent});
    if (!recipientInfo) throw new CreateTransferError('recipient not found');
    const tokenMetadata: MoveValue[] = await resolveTokenAddress(customToken as AccountAddress, client, symbol);
