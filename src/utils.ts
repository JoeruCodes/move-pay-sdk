import { AccountAddress, Aptos, GetFungibleAssetMetadataResponse, MoveFunctionId, MoveUint64Type, MoveValue } from "@aptos-labs/ts-sdk";
...
export async function resolveTokenAddress(tokenAddress: AccountAddress, client: Aptos, tokenSymbol?: string): Promise<MoveValue[]>{
    const is_object = await client.view({
        payload:{
            function: `${DEPLOYER_ADDRESS}::deployer::object_exists` as MoveFunctionId,
    ...
    if (is_object[0] as boolean) {
            const ret = await client.view({
                payload:{
                    function: `${DEPLOYER_ADDRESS}::deployer::get_metadata_from_asset_address` as MoveFunctionId,
                    functionArguments: [tokenAddress]
                } 
            });
...
export async function resolveTokenAddressNew(tokenAddress: AccountAddress, client: Aptos, tokenSymbol?: string): Promise<GetFungibleAssetMetadataResponse>{
    try{
        const fungibleAssetMetdata = await client.fungibleAsset.getFungibleAssetMetadataByCreatorAddress({
            creatorAddress: tokenAddress
        });
        return fungibleAssetMetdata;
    }catch(err){
        if(tokenSymbol){
            const fugibleAssetMetadata = await getTokenMetadata(tokenAddress, client, tokenSymbol)
...
async function getTokenMetadata(ownerAddress: AccountAddress, client: Aptos, symbol: string): Promise<MoveValue[]>{
    const ret = await client.view({
        payload:{
...
export function encryptPayload(qrPayload: string, secretKey: Uint8Array, linkShortner?: Function) {
