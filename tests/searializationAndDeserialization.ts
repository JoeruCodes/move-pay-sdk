...
import { LinkerPayloadInterface, petraQRConnect, petraQRConnectDApp } from "../src/wallet/petra";
...
describe('Serialization and Deserialization tests', () => {
    test('test serialization and deserialization tasks normal', () => {
        createTransfer(aptos, {
            recipent: recipent.accountAddress,
            amount: BigNumber(1)
        }).then((payload) => {
            const qrPayload = petraQRConnect(payload);
            console.log(qrPayload.toString().length);
    ...
    test('test serialization and deserialization tasks M2M DApp', () => {
        createTransfer(aptos,{ recipent: recipent.accountAddress, amount: BigNumber(1)}).then((payload) => {
            const qrPayload = petraQRConnectDApp(payload);
