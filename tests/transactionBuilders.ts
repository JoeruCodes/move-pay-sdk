    ...
    try{
        const txn = await aptos.transaction.build.simple({
            ...
            data:{
                ...
                functionArguments: ["SCOTOKEN", "SCO", 8, 69, "www.example.com", "www.example.com"]
            }
        });
...
describe('testing transaction builder', () => {
    ...
    test('test createTransfer with custom token', async () => {
        ...
        const tokenMetadata = await resolveTokenAddress(recipent.accountAddress as AccountAddress, aptos, "SCO");
        ...
        expect(ret).toEqual({
            data:{
                ...
                functionArguments: [fixMetadata(tokenMetadata), recipent.accountAddress,  1],
            },
            withFeePayer: true,
            message: undefined
        });
    ...
    test('test createSystemTransaction', async () => {
        ...
        const ret = await createSystemInstruction(aptos, recipent.accountAddress, BigNumber(1));
        expect(ret).toEqual({
            data:{
                function: "0x1::coin::transfer",
                typeArguments: ["0x1::aptos_coin::AptosCoin"],
    ...
    test('test create custom token transfer function', async () => {
        const ret = await createCustomTokenInstruction(recipent.accountAddress, BigNumber(1), recipent.accountAddress, aptos, "SCO");
        const tokenMetadata = await resolveTokenAddress(recipent.accountAddress, aptos, "SCO");
        expect(ret).toEqual({
            data:{
                function: "0x1::primary_fungible_store::transfer",
                typeArguments: ["0x1::fungible_asset::Metadata"],
                functionArguments: [fixMetadata(tokenMetadata), recipent.accountAddress, 100000000],
            },
            withFeePayer: true,
            message: undefined
        })
